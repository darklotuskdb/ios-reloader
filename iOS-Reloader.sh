...
echo -e "\e[92m[+] Preparing the cydia.list\e\0m"

mv /etc/apt/sources.list.d/cydia.list cydia.list.bak
...
if [ -f "$install_log" ]; then
  ...
  if cat "$install_log" | grep -ia "Unable to locate package" &> /dev/null ; then
    echo -e "\e[31m[-] The following packages are not installed properly. Please install manually:\e[0m"
...
while true; do
  read -p "Do you want to Respring your iOS device? (yes/no): " answer
  case "$answer" in
    [Yy][Ee][Ss])
